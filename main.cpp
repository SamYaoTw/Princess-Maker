...
int main() {
	...
	for (day = 1; day <= MAXday; day++)
	{
		//顯示
			...
			#建議可以多寫個DisplayAttributeDB()函數來依序呼叫DisplayAttribute()來顯示所有能力, 因為同樣的功能在結局也會用到
			for (int i = 1; i < player.size; i++)
			{
				#這裡的DisplayAttribute()中的Attribute*要加const較為妥當
				DisplayAttribute(player.PA+i);		//能力
			}
			printf("==================================================================\n"  );		//分隔線
		...
		//輸入&判斷
			...
			if (check == 1 && 1 <= choice && choice <= placedb.size)
			{
				...
				if (Qualified)
				{
					...
					{
						#由於沒有對應的函數給我使用, 所以我就直接在main()中直接做能力的搜尋		//好想用Trie啊 (X (<-你滾
						int attributeIndex;
	...
	//遊戲結局
		...
		//顯示能力
		#這裡一樣先假設"$"的能力在PA[0], 所以i從1開始
		#如上面所說, 因功能重複, 所以建議寫成DisplayAttributeDB()函數來呼叫
		for (int i = 1; i < player.size; i++)
		{
			#這裡的DisplayAttribute()中的Attribute*要加const較為妥當
			DisplayAttribute(&player.PA+i);		//能力
